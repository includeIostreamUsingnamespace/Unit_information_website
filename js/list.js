...
treeMenuLinks.forEach(function (link) {
    link.addEventListener('click', function (e) {
        ...
        if (data) {
            ...
            // 如果未找到对应数据，显示默认提示信息
            titleElement.textContent = 'JS提示：未找到对应信息标题';
            publishDateElement.textContent = 'JS提示：未定义发布日期';
            viewsElement.textContent = 'JS提示：未定义浏览量';
            docNumberElement.textContent = 'JS提示：未定义文号';
            publisherElement.textContent = 'JS提示：未定义发布单位';
            articleContentElement.textContent = 'JS提示：未找到对应信息内容';
        }
    });
});
